...
All the __Read MIAM XML__ is written in Python language.
### What the box does ?

This part box of the Workflow nammed  __Read MIAM XML__ is for (as its name suggests) the input XML verifiying and reading. We must therefore retrieve the values that interest us.
...
*__Read MIAM XML__ box is preceded by the __Create Folders__ box which consist in the repository creation (contained in the *namesOfFolder* agregate) and followed by the __Get Mesh & Models Frequencies__ box which consist in the recovery of the linear frequencies and the mesh frequencies (this is just, in fact linear frequencies to which we applied a step).*
...
To execute the script __Read MIAM XML__ correctly, we need to import Python libraries.
